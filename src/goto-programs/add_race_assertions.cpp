...
public:
  ...
  const symbolt &get_guard_symbol(const irep_idt &object, const exprt &original_expr)
  {
    ...
    new_symbol.type = original_expr.is_index()? migrate_type_back(index)
                                              : typet("bool");
  ...
  const exprt get_guard_symbol_expr(const irep_idt &object, const exprt &original_expr)
  {
    exprt expr = symbol_expr(get_guard_symbol(object, original_expr));

    if(original_expr.is_index())
    {
