  ...
  const exprt &original_expr)
{
  ...
  else if(expr.id() == "member")
  {
    ...
    read_write_rec(expr.op0(), r, w, "." + component_name + suffix, guard, expr);
  }
  ...
  {
    assert(expr.operands().size() == 1);
    read(expr.op0(), guard, original_expr);
  ...
  else if(expr.id() == "if")
  {
    ...
    read_write_rec(expr.op1(), r, w, suffix, true_guard, expr);

    guardt false_guard(guard);
    migrate_expr(gen_not(expr.op0()), tmp_expr);
    false_guard.add(tmp_expr);
    read_write_rec(expr.op2(), r, w, suffix, false_guard, expr);
  }
  else
  {
    forall_operands(it, expr)
      read_write_rec(*it, r, w, suffix, guard, expr);
  }
}
